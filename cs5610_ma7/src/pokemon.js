...
router.post("/", (req, res) => {
  ...
  if (foundPokemon)
    return res.send({ error: name + " already exists in the list" });

  const randomUUID = uuid();

  const newLevel =
    level === undefined ? Math.floor(Math.random() * 10) + 1 : level;
  const newHealth =
    health === undefined ? Math.floor(Math.random() * 91) + 10 : health;
...
router.put("/:pokemonId", function (req, res) {
  ...
  if (idx >= 0) {
    myPokemon[idx] = {
      ...
      health:
        health !== undefined
          ? parseInt(req.body.health)
          : myPokemon[idx].health,
      level:
        level !== undefined ? parseInt(req.body.level) : myPokemon[idx].level,
    };
